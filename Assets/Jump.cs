...
public class Jump : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if(currentCube == LevelGenerator.cubes.Count - 1) { Debug.Log("Success"); return; }
    ...
    private IEnumerator MoveAlongCurve(Vector3 jumpPosition)
    {
        ...
        Vector3 rotationAngle = LevelGenerator.cubes[currentCube + 1].transform.rotation.eulerAngles - LevelGenerator.cubes[currentCube].transform.rotation.eulerAngles;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.CompareTag("Dead"))
        {
            ...
            transform.position = LevelGenerator.cubes[currentCube].transform.position + new Vector3(0, LevelGenerator.cubes[currentCube].transform.lossyScale.y, 0) + 1 * transform.up;
        }
    }
